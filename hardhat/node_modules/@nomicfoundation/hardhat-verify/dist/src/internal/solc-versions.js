 ...
 */
export function resolveSupportedSolcVersions({ compilers, overrides, }) {
    ...
    const unsupportedSolcVersions = solcVersions.filter((version) => !semver.satisfies(version, SUPPORTED_SOLC_VERSION_RANGE));
    if (unsupportedSolcVersions.length > 0) {
        throw new HardhatError(HardhatError.ERRORS.HARDHAT_VERIFY.GENERAL.SOLC_VERSION_NOT_SUPPORTED, {
 ...
 */
export function filterVersionsByRange(versions, range) {
    return versions.filter((version) => semver.satisfies(version, range));
}
//# sourceMappingURL=solc-versions.js.map
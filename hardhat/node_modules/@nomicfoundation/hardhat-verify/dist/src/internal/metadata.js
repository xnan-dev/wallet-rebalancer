 ...
 */
export async function inferSolcVersion(bytecode) {
    let solcMetadata;
    try {
    ...
    if (solcMetadata instanceof Uint8Array) {
        ...
        log(`Unexpected metadata version format: [${[...solcMetadata].join(", ")}]`);
    }
 ...
 * the bytecode.
 */
export function getMetadataSectionBytesLength(bytecode) {
    const view = new DataView(bytecode.buffer, bytecode.byteOffset + bytecode.byteLength - METADATA_LENGTH_FIELD_SIZE, METADATA_LENGTH_FIELD_SIZE);
...
async function decodeSolcMetadata(bytecode) {
    ...
    const metadataPayload = bytecode.slice(-metadataSectionBytesLength, -METADATA_LENGTH_FIELD_SIZE);
    log(`Read metadata section length: ${metadataSectionBytesLength} bytes`);
    const lastBytes = metadataPayload.slice(-100);
    log(`Last ${lastBytes.length} bytes of metadata: ${bytesToHexString(lastBytes)}`);

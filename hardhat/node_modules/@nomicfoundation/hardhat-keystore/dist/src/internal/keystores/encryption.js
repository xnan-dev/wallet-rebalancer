...
// Constants
/// ////////////////////////////////////////////////////////////////////////////
export const KEYSTORE_VERSION = "hardhat-v3-keystore-1";
export const PASSWORD_NORMALIZATION_FORM = "NFKC";
...
export const KEY_DERIVATION_ALGORITHM = "scrypt";
export const KEY_DERIVATION_PARAM_N = 131_072;
export const KEY_DERIVATION_PARAM_R = 8;
export const KEY_DERIVATION_PARAM_P = 1;
export const KEY_DERIVATION_SALT_LENGTH_BYTES = 32;
export const MASTER_KEY_LENGTH_BITS = 256;
// HMAC-SHA-256
export const HMAC_ALGORITHM = "HMAC-SHA-256";
export const HMAC_KEY_LENGTH_BITS = 256;
// AES-GCM-SIV used for tolerance of IV collisions
export const DATA_ENCRYPTION_ALGORITHM = "AES-GCM-SIV";
export const DATA_ENCRYPTION_KEY_LENGTH_BITS = 256;
export const DATA_ENCRYPTION_IV_LENGTH_BYTES = 12;
// /////////////////////////////////////////////////////////////////////////////
 ...
 */
export function serializeEncryptedData(data) {
    return {
 ...
 */
export function deserializeEncryptedData(serializedData) {
    return {
...
export class UnsupportedTypeInDeterministicJsonError extends CustomError {
    type;
    constructor(type) {
        super(`Unsupported type in deterministicJson: ${type === "object" ? "array or null" : type}`);
        this.type = type;
    }
}
export class DecryptionError extends CustomError {
    constructor(cause) {
        super("Decryption failed: make sure you are using the right password/key and that your encrypted data isn't corrupted", cause);
    }
}
...
export class HmacKeyDecryptionError extends CustomError {
    constructor(cause) {
        super("Invalid hmac key: make sure you are using the right password/key and that your encrypted data isn't corrupted", cause);
    }
}
 ...
 */
export function encryptUtf8String({ encryptionKey, value, }) {
    const iv = randomBytes(DATA_ENCRYPTION_IV_LENGTH_BYTES);
    const cypherText = siv(encryptionKey, iv).encrypt(new TextEncoder().encode(value));
 ...
 */
export function decryptUtf8String({ encryptionKey, data, }) {
    let decryptedBuffer;
    try {
 ...
 */
export function createMasterKey({ password }) {
...
export function createEmptyEncryptedKeystore({ masterKey, salt, }) {
    ...
    const hmacPreImageObject = {
        ...
        hmacKey: serializeEncryptedData(encryptUtf8String({
            encryptionKey: masterKey,
            value: bytesToHex(hmacKey),
        })),
        dataEncryptionKey: serializeEncryptedData(encryptUtf8String({
            encryptionKey: masterKey,
    ...
    return {
        ...hmacPreImageObject,
        hmac: bytesToHex(generateEncryptedKeystoreHmac({
            masterKey,
 ...
 */
export function deriveMasterKeyFromKeystore({ password, encryptedKeystore, }) {
 ...
 */
export function doesKeyExist({ masterKey, encryptedKeystore, key, }) {
 ...
 */
export function addSecretToKeystore({ masterKey, encryptedKeystore, key, value, }) {
    validateHmac({ masterKey, encryptedKeystore });
    const dataEncryptionKey = hexToBytes(decryptUtf8String({
        encryptionKey: masterKey,
        data: deserializeEncryptedData(encryptedKeystore.dataEncryptionKey),
    }));
    const secrets = {
        ...encryptedKeystore.secrets,
        [key]: serializeEncryptedData(encryptUtf8String({ encryptionKey: dataEncryptionKey, value })),
    };
 ...
 */
export function removeSecretFromKeystore({ masterKey, encryptedKeystore, keyToRemove, }) {
    validateHmac({ masterKey, encryptedKeystore });
 ...
 */
export function decryptSecret({ masterKey, encryptedKeystore, key, }) {
    validateHmac({ masterKey, encryptedKeystore });
 ...
 * @returns The hmac.
 */
export function generateEncryptedKeystoreHmac({ masterKey, encryptedKeystore, }) {
    let hmacKey;
    try {
 ...
 */
export function validateHmac({ masterKey, encryptedKeystore, }) {

...
export function calculateDeploymentCompleteDisplay(uiState) {
    ...
    switch (uiState.result.type) {
        case DeploymentResultType.SUCCESSFUL_DEPLOYMENT: {
            const isRerunWithNoChanges = isResumed && !wasAnythingExecuted(uiState);
...
function _displaySuccessfulDeployment(result, { moduleName, isRerunWithNoChanges, deploymentDir, }) {
    const fillerText = isRerunWithNoChanges
        ? `Nothing new to deploy based on previous execution stored in ${pathFromCwd(deploymentDir ?? "Not provided")}`

...
async function runUserConfigExtensions(hooks, config) {
    return await hooks.runHandlerChain("config", "extendUserConfig", [config], async (c) => {
        return c;
    });
}
async function resolveUserConfig(projectRoot, configPath, hooks, sortedPlugins, config) {
    ...
    return await hooks.runHandlerChain("config", "resolveUserConfig", [config, (variable) => resolveConfigurationVariable(hooks, variable)], async (_, __) => {

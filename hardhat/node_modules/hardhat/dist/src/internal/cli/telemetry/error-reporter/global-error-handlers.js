...
function createUnhandledErrorListener(isPromiseRejection) {
    ...
    const mechanismType = isPromiseRejection
        ...
        : "onuncaughtexception";
    async function listener(error) {
        log(description, error);
        const telemetryError = error instanceof Error
            ? error
            : new Error(isObject(error) &&
                "message" in error &&
        ...
        try {
            ...
            log("Failed to send telemetry for unhandled error", telemetryErrorReportingError);
        }
        console.error();
 ...
 */
export function setupGlobalUnhandledErrorHandlers() {

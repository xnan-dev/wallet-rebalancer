 ...
 */
export function createDetachedProcessTransport(dsn, release, environment, getConfigPath) {
    return {
        send: (envelope) => {
 ...
 * exiting the process.
 */
export function createHttpTransport(dsn) {
    return createTransport({
        recordDroppedEvent(reason, category, count) {
            log(`Dropped event: ${reason} (category: ${category} - count: ${count})`);
        },
    }, async (request) => {
        try {
            log(`Sending envelope to Sentry backend using the HttpTransport`);
            const response = await sendSerializedEnvelopeToSentryBackend(dsn, request.body);
            log(`Successfully sent envelope to Sentry backend using the HttpTransport`);
            return response;
        }
        catch (e) {
            log(`Failed to send envelope to Sentry backend using the HttpTransport`, e);
            throw e;
        }
    });
}
 ...
 */
export async function sendEnvelopeToSentryBackend(dsn, envelope) {
    return await sendSerializedEnvelopeToSentryBackend(dsn, serializeEnvelope(envelope));
}
function isTsxRequiredForSubprocess(subprocessPath) {
...
async function sendSerializedEnvelopeToSentryBackend(dsn, serializedEnvelope) {
    const { hostname, username: publicKey, password: secret, pathname, } = new URL(dsn);

 ...
 */
const compiledConfigFile = new Map();
/**
 ...
 */
export async function resolveHardhatConfigPath(userProvidedPath) {
 ...
 */
export async function findClosestHardhatConfig(from) {
 ...
 */
export async function importUserConfig(configPath) {
    const normalizedPath = await normalizeConfigPath(configPath);
    const imported = await importConfigFileWithTsxFallback(pathToFileURL(normalizedPath).href);
    if (!("default" in imported)) {
        throw new HardhatError(HardhatError.ERRORS.CORE.GENERAL.NO_CONFIG_EXPORTED, {
            configPath,
        });
    }
    const config = imported.default;
    if (!isObject(config)) {
        throw new HardhatError(HardhatError.ERRORS.CORE.GENERAL.INVALID_CONFIG_OBJECT, {
            configPath,
        });
    }
    return config;
 ...
 */
async function normalizeConfigPath(configPath) {
    const normalizedPath = resolveFromRoot(process.cwd(), configPath);
    if (!(await exists(normalizedPath))) {
        throw new HardhatError(HardhatError.ERRORS.CORE.GENERAL.INVALID_CONFIG_PATH, {
            configPath,
        });
    }
        ...
        switch (error.name) {
            case "TransformError":
                const errors = error.message
                    ...
                    .map((line) => line.split(":"))
                    .map(([_path, line, _char, _code, ...message]) => `* Syntax error in line ${line}: ${message.join(":").trim()}`);
                throw new HardhatError(HardhatError.ERRORS.CORE.GENERAL.INVALID_CONFIG_FILE, {
                    configPath,
                    errors: `\t${errors.join("\n\t")}`,
                });
            default:
